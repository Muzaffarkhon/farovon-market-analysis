...
function openNavMenu(){
  ...
  el.innerHTML = '<div class="menu-pop">'+
    '<div class="menu-pop-hd"><div style="display:flex;align-items:center;gap:8px"><b>'+esc(userLabel())+'</b><span class="sheet-ver-badge">'+(window.APP_VERSION || 'v2.5.262')+'</span></div>'+
...
function openNavSubmenu(item){
  ...
  el.innerHTML = '<div class="nav-submenu-pop" role="menu">'+
    '<div class="nav-submenu-hd"><div style="display:flex;align-items:center;gap:8px">'+ic(item.icon, 14)+esc(item.label)+'<span class="sheet-ver-badge">'+(window.APP_VERSION || 'v2.5.262')+'</span></div>'+
      '<button class="menu-x" data-x="1" aria-label="Закрыть">'+icBare('close', 16)+'</button></div>'+
    '<div class="menu">'+
...
function openProfile(){
  ...
  el.innerHTML = '<div class="sheet-in profile-sheet">'+
    '<div class="sheet-hd"><div style="display:flex;align-items:center;gap:8px"><b>Профиль</b><span class="sheet-ver-badge">'+(window.APP_VERSION || 'v2.5.262')+'</span></div>'+
    ...
    '<div class="profile-ver">Обзор рынка вознаграждений · Фаровон · '+(window.APP_VERSION || 'v2.5.262')+'</div>'+
    '</div>';
  document.body.appendChild(el);
...
function renderAdminUsers(){
  ...
  // Кнопку добавления показываем только суперадмину (login «admin») — сервер
  // всё равно отклонит создание от кого угодно другого (см. adminController.saveUser).
  var canAddUser = !!(S.data && S.data.user && String(S.data.user.login).toLowerCase() === 'admin');
  var h = '<div class="toolbar">'+
    '<div class="search-wrap">'+icBare('search')+
    ...
    (canAddUser ? '<button id="btnAddUser" class="btn-primary toolbar-act">+ Добавить пользователя</button>' : '')+
  '</div>';
