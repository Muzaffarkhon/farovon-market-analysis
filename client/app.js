...
function navModel(){
  ...
  if(typeof canSeeGrading === 'function' && canSeeGrading()){
    var grSubs = [
      { key:'grading:assess', label:'Оценка должностей', icon:'grades', run:function(){ openGrading('assess'); } },
      { key:'grading:stats', label:'Сводка по грейдам', icon:'chart', run:function(){ openGrading('stats'); } }
    ];
    primary.push({ key:'grading', label:'Грейдирование должностей', icon:'grades',
  ...
  if(typeof canSeeKeyRisks === 'function' && canSeeKeyRisks()){
    ...
    primary.push({ key:'keyrisk', label:'Риски ключевого персонала', icon:'risk',
...
function openNavMenu(){
  ...
  el.innerHTML = '<div class="menu-pop">'+
    '<div class="menu-pop-hd"><div style="display:flex;align-items:center;gap:8px"><b>'+esc(userLabel())+'</b><span class="sheet-ver-badge">'+(window.APP_VERSION || 'v2.5.25')+'</span></div>'+
...
function openNavSubmenu(item){
  ...
  el.innerHTML = '<div class="nav-submenu-pop" role="menu">'+
    '<div class="nav-submenu-hd"><div style="display:flex;align-items:center;gap:8px">'+ic(item.icon, 14)+esc(item.label)+'<span class="sheet-ver-badge">'+(window.APP_VERSION || 'v2.5.25')+'</span></div>'+
      '<button class="menu-x" data-x="1" aria-label="Закрыть">'+icBare('close', 16)+'</button></div>'+
    '<div class="menu">'+
...
function openProfile(){
  ...
  el.innerHTML = '<div class="sheet-in profile-sheet">'+
    '<div class="sheet-hd"><div style="display:flex;align-items:center;gap:8px"><b>Профиль</b><span class="sheet-ver-badge">'+(window.APP_VERSION || 'v2.5.25')+'</span></div>'+
    ...
    '<div class="profile-ver">Обзор рынка вознаграждений · Фаровон · '+(window.APP_VERSION || 'v2.5.25')+'</div>'+
    '</div>';
  document.body.appendChild(el);
