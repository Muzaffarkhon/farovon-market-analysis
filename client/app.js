...
function openNavMenu(){
  ...
  el.innerHTML = '<div class="menu-pop">'+
    '<div class="menu-pop-hd"><div style="display:flex;align-items:center;gap:8px"><b>'+esc(userLabel())+'</b><span class="sheet-ver-badge">'+(window.APP_VERSION || 'v2.5.73')+'</span></div>'+
...
function openNavSubmenu(item){
  ...
  el.innerHTML = '<div class="nav-submenu-pop" role="menu">'+
    '<div class="nav-submenu-hd"><div style="display:flex;align-items:center;gap:8px">'+ic(item.icon, 14)+esc(item.label)+'<span class="sheet-ver-badge">'+(window.APP_VERSION || 'v2.5.73')+'</span></div>'+
      '<button class="menu-x" data-x="1" aria-label="Закрыть">'+icBare('close', 16)+'</button></div>'+
    '<div class="menu">'+
...
function openProfile(){
  ...
  el.innerHTML = '<div class="sheet-in profile-sheet">'+
    '<div class="sheet-hd"><div style="display:flex;align-items:center;gap:8px"><b>Профиль</b><span class="sheet-ver-badge">'+(window.APP_VERSION || 'v2.5.73')+'</span></div>'+
    ...
    '<div class="profile-ver">Обзор рынка вознаграждений · Фаровон · '+(window.APP_VERSION || 'v2.5.73')+'</div>'+
    '</div>';
  document.body.appendChild(el);
...
function openBatchSurveySheet(posName){
  ...
  if(!actualCos.length){
    if(S.editingPeriodId){
      toast('Для этой должности не выбрано ни одной компании для сравнения');
      return;
    }
    openPositionCompaniesSheet(posName);
    return;
  }
