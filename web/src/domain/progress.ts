...
/**
 * Прогресс одной записи — 9 пунктов. От списка ТЗ 3.4 отличается одним: вместо
 * «прочих выплат» (свободный текст, у большинства компаний законно пуст — ради
 * 100% пришлось бы писать «нет») считается график работы, который и так
 * обязателен при сохранении. Каждый пункт обязан совпадать с галочкой своего
 * блока в форме, иначе выходит «все блоки зелёные, а 8 из 9».
 */
...
const POINT_LABELS = [
  'оклад от', 'оклад до', 'график', 'есть ли премии', 'параметры премии',
  'льготы', 'источник', 'надёжность', 'комментарий'
];
