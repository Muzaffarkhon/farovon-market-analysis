...
async function runSeed() {
  ...
  // 4. Конкуренты — список компаний-конкурентов подразделения (по-прежнему
  // источник чек-листа position-first Шага 1). Колонка actual — наследие
  // старого унитарного Шага 1, системой больше не читается, но сохраняется
  // для обратной совместимости со старыми выгрузками seedBundle.json.
  // position_company_selections сознательно не сидируется — «ничего не
  // выбрано» это валидное стартовое состояние новой модели («не начата»).
  await insertMany(
    'Конкуренты',
    bundle.competitors,
