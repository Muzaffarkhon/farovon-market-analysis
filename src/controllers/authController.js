...
async function getUserPayload(user) {
  ...
  return {
    ...
    // Тип/приоритет конкурента — фиксированный список (в отличие от companies/
    // positions выше это не пользовательский справочник), но фронт (rowCard,
    // openAddSheet) безусловно читает S.data.ref.types/priorities при рендере
    // каждой строки конкурента. Это поле выпало при переносе с Apps Script на
    // Node/Express: без него открытие любого подразделения с конкурентами
    // падает с TypeError и вкладка "Конкуренты" остаётся пустой.
    ref: {
