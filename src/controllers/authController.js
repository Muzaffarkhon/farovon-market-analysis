...
async function getUserPayload(user) {
  ...
  return {
    ...
    ref: {
      ...
      // «Сдельно (за услугу)» — оплата за выполненную услугу/изделие, а не за
      // отработанное время. В месячные вилки и медиану такие ставки не
      // попадают (см. isPieceRate в analyticsService): сколько услуг выйдет за
      // месяц — неизвестно, и ставка за услугу исказила бы сравнение окладов.
      payPeriods: ['в час (ЧТС)', 'в день', 'в месяц', 'в год', 'сдельно (за услугу)'],
