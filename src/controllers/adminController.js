...
/**
 * Устойчивый идентификатор строки: одна и та же пара «подразделение +
 * компания» всегда даёт один cid, поэтому повторный запуск не плодит дубли
 * даже если INSERT OR IGNORE почему-то не сработает.
 */
...
/**
 * Раздача компаний по направлениям.
 *
 * Сейчас 715 строк участников рынка лежат всего на 72 подразделениях из 326:
 * так было в исходной таблице, компании заводили только у крупных отделов.
 * Если компания относится к направлению, она релевантна всем подразделениям
 * этого направления — иначе руководителю мелкого отдела нечего заполнять.
 *
 * Операция крупная: добавляется около 7400 строк, вдесятеро больше нынешнего.
 * Поэтому она двухшаговая — сначала возвращает расклад по направлениям и
 * ничего не трогает, и только с confirm выполняет вставку. Новые строки
 * помечаются src = «по направлению» и статусом «уточнить»: их никто не
 * проверял, и по этой метке их можно убрать обратно.
 */
async function distributeCompanies(confirm) {
  ...
  const comps = await queryAll(
    "SELECT name, segment, region, COALESCE(dirs,'') AS dirs FROM dictionary_companies WHERE TRIM(COALESCE(dirs,'')) <> ''");
  const existing = await queryAll('SELECT unit, company FROM competitors');

  const have = new Set(existing.map(r => r.unit + '|' + r.company));
  const byDir = new Map();
  comps.forEach(c => {
    String(c.dirs).split(';').map(s => s.trim()).filter(Boolean).forEach(d => {
  ...
  for (const d of divs) {
    const list = byDir.get(String(d.dir).trim());
    if (!list) continue;
    for (const c of list) {
      const key = d.unit + '|' + c.name;
      if (have.has(key)) continue;
      have.add(key); // одна и та же компания может быть в двух направлениях
      planned.push({ d, c });
      perDir.set(d.dir, (perDir.get(d.dir) || 0) + 1);
    }
  }

  const top = [...perDir.entries()].sort((a, b) => b[1] - a[1]).slice(0, 5)
    .map(([d, n]) => `${d} — ${n}`).join('; ');
  ...
  if (!confirm) {
    return {
      ...
      message: planned.length
        ? `Будет добавлено строк: ${planned.length} по ${perDir.size} направлениям ` +
          `(сейчас в базе ${existing.length}). Больше всего: ${top}. ` +
          'Все новые строки получат статус «уточнить» и метку «по направлению» — ' +
          'их можно будет убрать одной кнопкой.'
        : 'Добавлять нечего: у всех подразделений уже есть компании своих направлений.'
    };
  }
  ...
  const stmts = planned.map(({ d, c }) => ({
    sql: `INSERT OR IGNORE INTO competitors
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'уточнить', ?)`,
    args: [autoCid(d.unit, c.name), d.unit, d.dir, d.resp || '', d.hrbp || '',
           c.name, c.segment || '', c.region || '', AUTO_SRC]
  }));
 ...
 * никто не касался: если руководитель уже отметил компанию актуальной или
 * оставил комментарий, строка остаётся — это уже его работа, а не наша вставка.
 */
