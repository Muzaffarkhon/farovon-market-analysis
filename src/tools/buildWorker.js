...
const seedPath = path.join(baseDir, 'src', 'data', 'seedBundle.json');
const htmlPath = path.join(baseDir, 'public', 'index.html');
const workerPath = path.join(baseDir, 'src', 'worker.js');

if (!fs.existsSync(seedPath)) {
  console.error('seedBundle.json not found, please run generateDataBundle.js first');
  process.exit(1);
}

const seedBundle = JSON.parse(fs.readFileSync(seedPath, 'utf8'));

// Ensure public/index.html starts with proper HTML5 header and charset
let htmlContent = fs.readFileSync(htmlPath, 'utf8');
if (!htmlContent.includes('<meta charset="UTF-8">') && !htmlContent.includes('<meta charset="utf-8">')) {
...
` + htmlContent + `
  ...
  console.log('✅ Updated public/index.html with UTF-8 doctype and meta headers');
}
...
// ─────────────────────────────────────────────────────────────
const INITIAL_DATA = ${JSON.stringify(seedBundle)};
  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (path === '/survey/save' && request.method === 'POST') {
        ...
        // Добавление новых записей конкурентов
        added.forEach(a => {
      ...
      if (path === '/dashboard/hrbp' && (request.method === 'POST' || request.method === 'GET')) {
        const data = getExtendedAnalytics(state, {});
      ...
      // Если запрос к / или любому другому маршруту страницы
      if (env.ASSETS) {
        return env.ASSETS.fetch(request);
      }

      // Fallback: отдаем встроенный HTML SPA
...
console.log('✅ Успешно скомпилирован src/worker.js (' + (workerCode.length / 1024).toFixed(1) + ' KB)');
`;

fs.writeFileSync(path.join(baseDir, 'src', 'tools', 'buildWorker.js'), workerCode, 'utf8');
console.log('Created buildWorker.js');
