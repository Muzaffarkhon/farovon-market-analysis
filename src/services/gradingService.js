 ...
 * одной группы нельзя было сравнить с другой. Теперь один набор факторов и
 * одна шкала на всех, чтобы влияние на бизнес было сравнимо между любыми
 * двумя должностями холдинга.
 *
 * Обновлено 2026-09-17: методика «Единая система оценки должностей
 * ГК «Фаравон»» — 7 факторов вместо 6 (добавлен К7 «Управление людьми и
 * ресурсами», см. config/gradingFactors.js) и шкала групп в порядке
 * возрастания (Группа I — исполнительский персонал, Группа V — высшее
 * руководство), а не убывания, как было раньше. Данные, накопленные до этой
 * миграции, физически удалены и требуют переоценки по новой анкете — та же
 * логика, что и при переходе 2026-09-14 (см. migrate.js).
 *
...
/** Веса 7 факторов — сумма 100%. Порядок совпадает с CRITERIA в config/gradingFactors.js. */
const CRITERIA_WEIGHTS = [0.10, 0.10, 0.15, 0.15, 0.15, 0.15, 0.20];
const FACTOR_COUNT = CRITERIA_WEIGHTS.length;

/** Самый младший уровень шкалы — ниже нижнего порога грейд не опускается. */
const MAX_GRADE = 1;

/**
 * Группы (грейды) по итоговому баллу — 5 равных интервалов шириной 0,80 от
 * 1,00 до 5,00. Номер группы растёт вместе со сложностью должности (1 —
 * Группа I, самый младший уровень; 5 — Группа V, высшее руководство).
 * Порядок массива — от старшей группы к младшей: calcGrade() возвращает
 * первую, чей порог снизу пройден.
 */
const GRADE_THRESHOLDS = [
  { grade: 5, from: 4.20, name: 'Группа V', label: 'Высшее руководство' },
  { grade: 4, from: 3.40, name: 'Группа IV', label: 'Руководители направлений и ключевые эксперты' },
  { grade: 3, from: 2.60, name: 'Группа III', label: 'Ведущие специалисты и руководители подразделений' },
  { grade: 2, from: 1.80, name: 'Группа II', label: 'Квалифицированные специалисты и супервайзеры' },
  { grade: 1, from: 1.00, name: 'Группа I', label: 'Исполнительский персонал' }
];
 ...
 * факторов (FACTOR_COUNT) — иначе оператор пропустил вопрос, и балл молча оказался бы
