...
test('грани каскадные по оргструктуре: направление сужает список подразделений', () => {
  const res = build(mixed, { dir: 'Дивизион Север' });
  // Собственное поле фильтра (dir) из своей грани не сужается — иначе выбранное
  // значение пропало бы из списка направлений и снять фильтр было бы нечем.
  assert.deepEqual(res.facets.dirs, ['Дивизион Север', 'Дивизион Юг']);
  // А вот подразделения — только из выбранного направления, не все три.
  assert.deepEqual(res.facets.units, ['Цех 1', 'Цех 2']);
});

test('грани каскадные и в обратную сторону: подразделение сужает список направлений', () => {
  const res = build(mixed, { unit: 'Цех 3' });
  assert.deepEqual(res.facets.dirs, ['Дивизион Юг']);
  assert.deepEqual(res.facets.units, ['Цех 1', 'Цех 2', 'Цех 3']);
});
