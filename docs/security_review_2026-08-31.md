...
- [ ] `if (targetRole === 'admin' && req.user.role !== 'admin') → 403`
- [ ] Запретить не-админу менять `role` и `active` у существующего пользователя с `role='admin'`
...
- [ ] `express-rate-limit` на `/api/auth/login` (~10 / 15 мин на IP+логин)
- [ ] `express-rate-limit` на `/api/telegram/webhook`
- [ ] Мягкий глобальный лимит на `/api`
- [ ] Счётчик неудачных входов + временная блокировка учётки (таблица/поле + проверка в `login`)
...
- [ ] При успешном входе по SHA-ветке — сразу перехэшировать в bcrypt и записать
- [ ] Разовый скрипт: отчёт, у кого ещё `password_hash` не `$2a/$2b/$2y`
...
- [ ] `cors({ origin: [<прод-URL фронта>, 'https://web.telegram.org'], credentials: false })`
...
- [ ] Убрать приём токена из query
- [ ] Для `/api/dashboard/export-csv` — отдельная короткоживущая подписанная ссылка вместо `?token=`
