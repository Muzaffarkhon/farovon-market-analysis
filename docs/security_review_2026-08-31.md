...
- [x] **Принудительная смена временного пароля** при первом входе: флаг users.must_change_password (ставит бот при выдаче временного, снимает changePassword); authMiddleware пускает такого юзера только на /auth/change-password и /auth/resume; фронт форсит немодальное окно смены без кнопки закрытия
