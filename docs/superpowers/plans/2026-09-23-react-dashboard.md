...
- [x] **Step 1: Тесты на новые разрезы — пишутся первыми, до реализации**
...
- [x] **Step 2: Прогнать — падает** (функций ещё нет)
...
- [x] **Step 4: Прогнать все серверные тесты**
...
- [x] **Step 1: Тест `ForkBar` — процентные позиции блока и риски**
...
- [x] **Step 2: Реализация** (KpiTile/ForkBar/RankBar + `forkLayout`)
...
- [x] **Step 1: Тест навигации** — пункт меню виден только при `dashboard:view`,
скрыт без него (расширение существующего `NavItems.test.ts`).

- [x] **Step 2: `useDashboard` и `DashboardScreen`**
...
- [x] **Step 1: Тест** — рендерит KPI-плитки из моков `dashboardApi`, топ-6
должностей строками, карточку рейтинга льгот.
- [x] **Step 2: Реализация** (по мокапу `04-dashboard.html`, раздел 4.3 спеки)
- [x] **Step 3: Прогнать, вручную сверить числа с текущим дашбордом старого клиента, commit**
...
- [x] **Step 1: Тест** — поиск по названию должности фильтрует список; клик
...
- [x] **Step 1: Тесты** — таблица регионов отсортирована по медиане; рейтинг
...
- [x] **Step 1: Тест** — вкладка не отображается в списке вкладок без
...
- [x] **Step 1: Тест** — две таблицы (`hrbpProgress`, `dirProgress`) с полосой
...
- [x] **Step 1: Прогнать всё** — `npm test` (сервер), `npm --prefix web test`,
`npm --prefix web run build`
- [x] **Step 2: Сквозная проверка (раздел 15 ТЗ, применимо к этому этапу)** —
...
- [x] **Step 3: Обновить опись** (если есть общий список пройденных этапов)
- [x] **Step 4: Commit**
