...
- [ ] **Step 1: Тесты хаба и списка пользователей**
...
- [ ] **Step 4: Прогнать, вручную на dev-данных (создать тестового
...
- [ ] **Step 2: Вкладка «Формулировки»**
- [ ] **Step 3: Вкладка «Блоки»** — перенос должности между блоками,
  сброс оценки
- [ ] **Step 4: Вкладка «Комиссия»** — состав по блоку, добавить/убрать по
  ФИО, список в ожидании кворума
- [ ] **Step 5: Прогнать, вручную (перенести тестовую должность между
...
- [ ] **Step 2: Вкладка «Источники»** — список, создать/редактировать, веса
- [ ] **Step 3: `BenchmarkImportWizard`** — четыре шага
- [ ] **Step 4: Вкладка «Сопоставление»** — таблица + автоподсказки
- [ ] **Step 5: Вкладка «Наборы данных»** — список, удаление
- [ ] **Step 6: Прогнать, вручную (импорт тестового xlsx на dev-данных,
...
### Task 6: Раздел «Периоды сбора» (+ новый серверный маршрут)

**Files:**
- Create: `src/controllers/adminController.js` (`listPeriods`, дописать),
  `web/src/api/periods.ts`, `web/src/features/admin/periods/*` + тесты, `usePeriods.ts`, css
- Modify: `src/routes/api.js` (`GET /admin/periods`), `contract.ts`
  (`PeriodRow`), `test/adminController*.test.js` (если есть подходящий
  файл для юнит-теста контроллера — иначе ручная проверка через dev-сервер,
  как и для прочих admin-маршрутов без чистых функций)

**Interfaces:** `periodsApi.list()`, `.setPeriod(action, payload)`,
`.grants()`, `.grant(...)`, `.revokeGrant(...)`, `.grantUsers()`, `.deletePeriod(id)`

- [ ] **Step 1: Серверный тест/проверка нового маршрута**

`GET /admin/periods` отдаёт все периоды, не только активный, отсортированные
по id убыв., с правом `period:view`.

- [ ] **Step 2: Реализация маршрута**
- [ ] **Step 3: Тесты клиента**

«Новый период» показывает предупреждение о сбросе `actual` (текст —
проверить и перенести дословно из старого клиента, не выдумывать); удаление
непустого периода показывает ошибку сервера как есть.

- [ ] **Step 4: Реализация экрана + панель грантов на архивный период**
- [ ] **Step 5: Прогнать, вручную (посмотреть список периодов на dev-данных,
...
```bash
git add -A
git commit -m "feat(web): раздел «Периоды сбора» + серверный список периодов"
```
