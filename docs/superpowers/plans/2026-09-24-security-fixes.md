...
- [x] **Step 4: Прогнать вручную (на dev-данных: войти в новый клиент,
  ...
  бота по-прежнему работают без токена), commit** — прогнано 24.09
  через fetch в браузерной сессии dev.admin вместо curl (то же самое:
  реальная сессионная кука + ручные заголовки): мутирующий toggle
  пользователя с `X-CSRF-Token` → 200, без заголовка → 403
  `CSRF_TOKEN_MISMATCH`; `/api/auth/login` без токена → 200 (маршрут
  исключён). Изменённое тестовыми запросами состояние (`dev.user.active`)
...
Обновить комментарий на месте (объяснить, почему сузили и когда). По
факту `users:create` убран из каталога прав целиком отдельным
коммитом раньше (добавление пользователей — только суперадмин, не
делегируется), здесь остались `users:edit`/`broadcast:send`/
`service:edit`.

- [x] **Step 2: Прогнать (`npm test` — существующие тесты на дефолтные
  ...
  (создание/правка) и «Рассылка»), commit** — `npm test` зелёный
  (229/229). Ручную dev-БД правку (`DELETE ... role_capabilities`)
  выполнить не смог — заблокирована защитным классификатором как
  прямая правка общих данных; пользователю оставлена инструкция
  перевыставить то же самое через «Роли и доступы» в интерфейсе.
...
- [x] **Step 1:** `npm test` (229/229), `npm --prefix web test` (48
  файлов / 276 тестов), `npm --prefix web run build` — все зелёные.
- [x] **Step 2:** Сквозная проверка на dev-сервере под login `dev.admin`:
  вход в старый клиент работает; `/new` открывается напрямую по URL
  (SPA-маршрут почтен отдельным фиксом раньше) без CSP-нарушений в
  консоли; мутирующий запрос (toggle пользователя) — 200 с
  `X-CSRF-Token`, 403 без него; `/api/auth/login` — 200 без токена
  (исключённый маршрут); кнопка «Добавить» в «Пользователи» скрыта у
  `dev.admin` (роль admin, но не встроенный суперадмин) — ограничение
  из отдельного коммита тоже держится под CSRF/CSP.
- [x] **Step 3:** Commit
